...
int render(SDL_Renderer* renderer, SDL_Texture* texture, SDL_Color* colors, 
           TTF_Font* font, const char* text, int* effect_state);
...
int game_setup(SDL_Window* window, SDL_Texture** texture, SDL_Texture* stage_backgrounds[], 
    ...
    *texture = load_texture("./graphics/title_screen.bmp", renderer);
    ...
    fonts[0] = load_font("./graphics/zalgo.ttf", 92);
    fonts[1] = load_font("./graphics/zalgo.ttf", 32);
    if (!fonts[0] || !fonts[1]) {
        fprintf(stderr, "Could not load font. Reason: %s\n", TTF_GetError());
...
int render(SDL_Renderer* renderer, SDL_Texture* texture, SDL_Color* colors, 
    ...
    float effect_inter = delta_time * 0.666f;
    ...
    renderText(renderer, "cygnus ... x-1", 460, 110, colors[2], font);
    renderText(renderer, "cygnus ... x-1", 455, 105, colors[1], font);
    renderText(renderer, "cygnus ... x-1", 445, 95, colors[3], font);
    renderText(renderer, "cygnus ... x-1", 450, 100, colors[0], font);

    // Render text if effect state is active
    if (*effect_state) {
        // Text rendering calls inside the render function
        renderText(renderer, "0001 ....", 108, 480, colors[2], font);
        renderText(renderer, "0001 ....", 100, 485, colors[1], font);
        renderText(renderer, "0001 ....", 91, 490, colors[0], font);
        renderText(renderer, ".... 0010", 879, 480, colors[2], font);
        renderText(renderer, ".... 0010", 876, 485, colors[1], font);
        renderText(renderer, ".... 0010", 867, 490, colors[0], font);
    }
...
int main(void) {
    ...
    if (!initSDL(&window, &renderer)) {
        fprintf(stderr, "Failed to initialize SDL. Exiting with grace...\n");
        return EXIT_FAILURE;
    }

    if (!game_setup(window, &texture, stage_backgrounds, renderer, fonts, colors, &effect_state)) {
        fprintf(stderr, "Failed to set up game. Cleaning up and exiting...\n");
        cleanup(window, renderer, texture, stage_backgrounds, fonts, NUM_FONTS);
        return EXIT_FAILURE;
    }
    ...
    while (game_running) {
        ...
        if (!render(renderer, texture, colors, fonts[0], "Cygnus X-1", &effect_state)) {
            printf("Could not pass renderer to render! Quitting!\n");
            break;
        }
    }
